...
class ECUSimulator:
    ...
    def _update(self):
        with self._lock:
            ...
            self.throttle_position = max(THROTTLE_MIN, min(THROTTLE_MAX, self.throttle_position))

            if not self._faults["overheat"]:
                self.rpm = round(
                    800 + (self.throttle_position / 100) * 5500
                    + random.uniform(-80, 80), 0
                )
                ...
                self.engine_temp = max(ENGINE_TEMP_MIN, min(ENGINE_TEMP_MAX, self.engine_temp))
            else:
                self.engine_temp = round(min(self.engine_temp + random.uniform(0.5, 2.0), 135.0), 1)
                self.rpm = round(max(self.rpm - random.uniform(50, 150), RPM_MIN), 0)

            if not self._faults["voltage_drop"]:
                self.battery_voltage = round(
                    12.6 + (self.rpm / RPM_MAX) * 1.8 + random.uniform(-0.05, 0.05), 2
                )
                self.battery_voltage = max(BATTERY_VOLTAGE_MIN, min(BATTERY_VOLTAGE_MAX, self.battery_voltage))
            else:
                self.battery_voltage = round(max(self.battery_voltage - random.uniform(0.05, 0.2), 10.0), 2)

            if not self._faults["sensor_corruption"]:
                self.fuel_pressure = round(
                    350 + random.uniform(-30, 30), 1
                )
                self.fuel_pressure = max(FUEL_PRESSURE_MIN, min(FUEL_PRESSURE_MAX, self.fuel_pressure))
            else:
                self.fuel_pressure = round(random.uniform(0, 600), 1)
